import { fetchAllGamesGenre, fetchTags } from "../services/rawApi"; // Asegúrate de que ambas funciones estén correctamente implementadas
...
export function TagsDetalles() {
    ...
    const [tagName, setTagName] = useState(""); // Estado para almacenar el nombre del tag
    const [error, setError] = useState(null);
    const [searchTerm, setSearchTerm] = useState(""); // Estado para el término de búsqueda
    const [pagina, setPagina] = useState(1); // Cambiado a 1 para empezar desde la primera página
    const [tags, setTags] = useState([]); // Estado para almacenar la lista de tags
    ...
    useEffect(() => {
        const obtenerJuegosPorTag = async () => {
            try {
                const data = await fetchAllGamesGenre(pagina, id); // Asegúrate de que esta función acepte el ID del tag
        ...
        const obtenerTags = async () => {
            try {
                const allTags = await fetchTags(); // Obtiene la lista de tags
                const tag = allTags.find(t => t.id === parseInt(id)); // Busca el tag por ID
                setTagName(tag ? tag.name : "Desconocido"); // Guarda el nombre o "Desconocido" si no se encuentra
            } catch (error) {
                setError(error.message);
            }
        };

        obtenerTags(); // Llama a la función para obtener la lista de tags
        obtenerJuegosPorTag(); // Llama a la función para obtener los juegos
        window.scrollTo(0, 0); // Desplaza la ventana hacia arriba
    }, [pagina, id]); // Ejecuta cada vez que cambia la página o el ID del tag
